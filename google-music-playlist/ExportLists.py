...
def playlist_handler(playlist_name, playlist_description, playlist_tracks):
    ...
    outfile = codecs.open(os.path.join(output_dir,playlist_name+u'.csv'), mode='wb')
    outfile.write(codecs.BOM_UTF16_LE)
    ...
    if playlist_description:
        outfile.write(tsep.encode('utf-16-le'))
        outfile.write(playlist_description.encode('utf-16-le'))
        outfile.write(os.linesep.encode('utf-16-le'))
    ...
    for tnum, pl_track in enumerate(playlist_tracks):
        ...
        outfile.write(create_details_string(result_details).encode('utf-16-le'))
        outfile.write(os.linesep.encode('utf-16-le'))
